...
const FormData = require('form-data');
     ...
     * @param platformEvent
     */
    function Postsmshistory(sfAuthReponse,platformEvent){
        var lastFive = phoneNumber.substr(phoneNumber.length - 4);
        console.log('Posting Platform Event:',platformEvent);

        var options = {
            uri: sfAuthReponse.instance_url + "/services/data/v44.0/sobjects/SMS_History__c",
            headers: {
        ...
        body:{"Contact__c":"yatheen23@gmail.com","Message_Type__c":"incoming;"+body},
    ...
    function postPlatformEvent(sfAuthReponse,platformEvent){
        ...
        body:{"Phone":phoneNumber,"LastName":"Customer-"+lastFive,"AssistantName":"Customer-"+lastFive,"promocode__c":coupon,"Source__c":"PinText"},
    ...
    function getresponsebygot()
    {
      ...
      console.log("length is :"+ attributes.records.length);
       if (attributes.records.length !== 0) {
         ...
         //Postsmshistory(sfAuthReponse,platformEvent);
      }
    })
    .catch(error => {
      console.error(error);
