    ...
    function processEventResponse(error, response, body) {
        ...
        if (!error && response.statusCode == 201) {
            console.log('Success Posting Platform Event:  Server responded with:', body);
              var message="ThankYou for chosing us. please use PromoCode "+couponcode;
            twiml.message(message);
            callback("",message);
        } else{
             var message="Thank you for choosing us. Show this code to SuperPlay on next visit: "+couponcode;
            twiml.message(message);
            callback("",message);
